        ...
        for i in range(len(atech)):
            if len(idxlist) < 100:
                if type(atech[i]) is str and type(anotice[i]) is str and type(aproblem[i]) is str and \
                        ...
                        type(aboss[i]) is str and type(anight[i]) is str and type(alegacy[i]) is str and type(
                    amars[i]) is str:
                    idxlist.append(indx)
...
def important():
    ...
    for region in regionset:
        ...
        for i in range(len(atech)):
            if len(idxlist) < 100:
                if type(ivariety[i]) is str and type(icontrol[i]) is str and type(isameend[i]) is str and \
                        ...
                        type(ipromotion[i]) is str and type(imission[i]) is str and type(iwfh[i]) is str and type(
                    ioffice[i]) is str:
                    idxlist.append(indx)
        ...
        if len(idxlist) > 0:
            ...
            avglist.append(
                [ivarietyavg, icontrolavg, isameendavg, inewtechavg, ibuildnewavg, ibuildexistingavg, ipromotionavg,
